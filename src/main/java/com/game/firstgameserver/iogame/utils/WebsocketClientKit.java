...
public class WebsocketClientKit {
    public void runClient() throws Exception {
        // 连接游戏服务器的地址
        String wsUrl = "ws://jnchou.top:10100/websocket";
        ...
        WebSocketClient webSocketClient = new WebSocketClient(new URI(wsUrl), new Draft_6455()) {
            ...
            public void onOpen(ServerHandshake handshakedata) {
                ...
                ClientCommandKit.listRequestExternalMessage().forEach(externalMessage -> {
                    ...
                    System.out.println("66+6666666" + base64Str);
                    this.send(bytes);

                    log.info("发送请求 {}-{}", cmd, subCmd);

                    if (ClientCommandKit.login) {
                        ClientCommandKit.login = false;
                        // 如果有登录，那么停1.5秒，等登录完成后在继续遍历请求
                        try {
